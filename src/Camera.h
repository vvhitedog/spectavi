...
template <typename MatrixType = RowMatrixXd> class Camera {

private:
  MatrixType m_P;

public:
  Camera(const Eigen::Ref<const MatrixType> &P) : m_P(P) {}
  ...
  Camera() { m_P = MatrixType::Identity(3, 4); }

  const MatrixType &get_matrix() const { return m_P; }
};
...
private:
  ...
  MatrixType fundamental() const {
    ...
    return skew_symmetric<MatrixType>(ep) * m_P1 *
           invP0; // The fundamental matrix of P0, P1
  }
  ...
  MatrixTypeI resample_line_idx(const Eigen::Ref<const MatrixType> &yy,
    ...
    for (int i = 0; i < xx.size(); ++i) {
      ...
      int _x = (int)x;
      int _y = (int)y;
      if (_x >= 0 && _x < im.cols() && _y >= 0 && _y < im.rows()) {
        resamp(i) = im(_y, _x);
      } else {
        resamp(i) = -1;
      }
    ...
    for (int i = 0; i < xx.size(); ++i) {
      ...
      if (_x >= 0 && _x < im.cols() / m_nchannels && _y >= 0 &&
        ...
        for (int ic = 0; ic < m_nchannels; ++ic) {
          resamp(m_nchannels * i + ic) = im(_y, m_nchannels * _x + ic);
        }
      } else {
        for (int ic = 0; ic < m_nchannels; ++ic) {
  ...
  std::pair<double, int>
  compute_shift_row_ncc(const Eigen::Ref<const MatrixType> &r0,
                        const Eigen::Ref<const MatrixType> &r1,
                        int shift) const {
    int nx0 = r0.size() / m_nchannels;
    int nx1 = r1.size() / m_nchannels;
    double sum2_0 = 0;
...
public:
  Rectifier(const Scalar *P0, const Scalar *P1, const Scalar *im0,
            const Scalar *im1, int rows0, int cols0, int rows1, int cols1,
            int nchannels)
      : m_P0(P0, 3, 4), m_P1(P1, 3, 4), m_im0(im0, rows0, cols0 * nchannels),
        m_im1(im1, rows1, cols1 * nchannels), m_nchannels(nchannels),
  ...
  Rectifier &resample(double sampling_factor) {
    ...
    for (int irow = -extra_rows; irow < largest_rows + extra_rows; ++irow) {
      ...
      origin << 0., irow, 1.;
      MatrixType seed(
          3, 1); // A point in im0 that will define the epipolar line in im1
    ...
    // Shift the right image consistently in columns to best align the two
    // images.
    const int k = 5;
    MatrixType sol;
    ...
    for (int itry = 0; itry < 0; ++itry) {
      ...
      std::vector<int> indices =
          draw_random_indices<std::vector<int>>(0, largest_rows, k);
      ...
      Eigen::JacobiSVD<MatrixType> svd(ys, Eigen::ComputeThinV |
                                               Eigen::ComputeThinU);
    ...
    m_output_cols = max_right - min_left; // reset output cols correctly
    {
      ...
      for (int irow = -extra_rows; irow < largest_rows + extra_rows; ++irow) {
        ...
        // int shift = sol(0) * irow + sol(1);
        int shift = 0;
        copy0.block(itarget_row, m_nchannels * (-min_left), 1, m_rim0.cols()) =
            m_rim0.row(itarget_row);
...

    {
      ...
      for (int irow = -extra_rows; irow < largest_rows + extra_rows; ++irow) {
        ...
        // int shift = sol(0) * irow + sol(1);
        int shift = 0;
        copy0.block(itarget_row, (-min_left), 1, m_idx0.cols()) =
            m_idx0.row(itarget_row);
        copy1.block(itarget_row, (-shift - min_left), 1, m_idx1.cols()) =
            m_idx1.row(itarget_row);
      }
      m_idx0 = copy0;
      m_idx1 = copy1;
    }
    ...
    //    int itarget_row = irow+zero_row; // shift indices to get to zero
    //    int shift =
    //    globally_align_rows(m_rim0.row(itarget_row),m_rim1.row(itarget_row));
    //    std::cout <<  " done rows: " << irow << " -> " << largest_rows +
    //    extra_rows << " shift: " <<
    //       shift  << std::endl;
    //}
    return *this;
  ...
  int rows() const { return m_output_rows; }
